...
export class TestDiscovery implements vs.Disposable {
    ...
    private async discoverItem(newItemForUri: Map<vs.Uri, vs.TestItem>, uri: vs.Uri, name?: string, type?: vs.FileType): Promise<vs.TestItem | undefined> {
        ...
        if (type === vs.FileType.File && uri.path.endsWith('.ht')) {
            ...
            const children = childFiles.map(([childName, childType]) => {
                return this.discoverItem(newItemForUri, vs.Uri.joinPath(uri, childName), childName, childType)
            })
            const definedChildren = (await Promise.all(children))
                .filter((c): c is vs.TestItem => c !== undefined)
...
function isWithinWorkspace(folder: string): boolean {
    return vs.workspace.workspaceFolders?.some(f => isWithinPath(folder, f.uri.fsPath)) ?? false;
}
