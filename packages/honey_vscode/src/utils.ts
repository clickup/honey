...
export async function  waitUntilExit(process: child_process.ChildProcess): Promise<number> {
	if (process.exitCode) {
...
export function runProcessInOutputChannel(process: ChildProcess, channel: vs.OutputChannel) {
